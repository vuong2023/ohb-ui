...
iplimit_log_path="${log_folder}/ohb.log"
iplimit_banned_log_path="${log_folder}/ohb-banned.log"
...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/vuong2023/ohb-ui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/vuong2023/ohb-ui/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
update_geo() {
    ...
    systemctl start x-ui
    echo -e "${green}Geosite.dat + Geoip.dat + geoip_IR.dat + geosite_IR.dat + geoip_VN.dat + geosite_VN.dat have been updated successfully in bin folder '${binfolder}'!${plain}"
    before_show_menu
}
